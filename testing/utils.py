...
# Model Initialization Functions (for multiprocessing safety)
...
API_CONFIG = {
    ...
    "cftc": "https://www.cftc.gov/rss/cftc_rss.xml",
}

# Dynamic Adjustment Cache
DYNAMIC_WEIGHTS = {
    "state_regulators": BASE_STATE_REGULATORS.copy(),
    "risk_categories": BASE_RISK_CATEGORIES.copy(),
    "region_terms": BASE_REGION_TERMS.copy(),
}
LAST_UPDATE = None

def fetch_dynamic_weights():
    """Fetch real-time data from APIs to adjust weights."""
    global DYNAMIC_WEIGHTS, LAST_UPDATE
    now = datetime.now()
    if LAST_UPDATE and (now - LAST_UPDATE) < timedelta(hours=24):
    ...
    def fetch_news():
        try:
            response = requests.get(API_CONFIG["news"], timeout=10)
            data = response.json()
            ...
            for article in articles:
                ...
                for cat, terms in BASE_RISK_CATEGORIES.items():
                    for term in terms:
                        if term in text:
                            risk_mentions[term] = risk_mentions.get(term, 0) + 1
    ...
    def fetch_sanctions():
        try:
            regions = set()
            for region in BASE_REGION_TERMS:
                response = requests.get(API_CONFIG["sanctions"].format(query=region), timeout=10)
                data = response.json()
                if data.get("results"):
                    regions.add(region)
            return regions
        except Exception as e:
            logging.error(f"Sanctions fetch failed: {e}")
            return set()

    def fetch_market():
        try:
            response = requests.get(API_CONFIG["market"], timeout=10)
            data = response.json()
            volatility = np.mean([coin.get("price_change_percentage_24h", 0) for coin in data])
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        ...
        sanctions_future = executor.submit(fetch_sanctions)
        market_future = executor.submit(fetch_market)

        risk_mentions = news_future.result()
        for cat, terms in dynamic_weights["risk_categories"].items():
            for term in terms:
                if term in risk_mentions:
                    terms[term] += min(2.0, risk_mentions[term] * 0.1)

        sanctioned_regions = sanctions_future.result()
        for region in dynamic_weights["region_terms"]:
            if region in sanctioned_regions:
                dynamic_weights["region_terms"][region] = min(7.0, dynamic_weights["region_terms"][region] + 1.5)

        market_volatility = market_future.result()
        for term in dynamic_weights["risk_categories"]["market_risk"]:
            dynamic_weights["risk_categories"]["market_risk"][term] += market_volatility

    DYNAMIC_WEIGHTS = dynamic_weights
    LAST_UPDATE = now
    logging.info("Dynamic weights updated from APIs")
    return DYNAMIC_WEIGHTS
...
def extract_company_names(text, edgar_client=None):
...
def extract_person_names(text, flair_tagger=None):
...
def extract_financial_terms(text):
    ...
    return {
        ...
        "risk_mentions": [risk for category in DYNAMIC_WEIGHTS["risk_categories"].values()
                          for risk in category if risk.lower() in text_lower]
    }

def calculate_risk_score(found_risks, sentiment_score, text_length, source_type="text", regions=None, text="", tokenomics_flags=None):
    """Enhanced risk score with dynamic weights and tokenomics."""
    weights = fetch_dynamic_weights()
    raw_score = 0.0
    categories_hit = set()
    text_lower = text.lower()
    ...
    for risk in found_risks:
        ...
        for cat, terms in weights["risk_categories"].items():
    ...
    sentiment_factor = 1.0 + (abs(sentiment_score) ** 1.5 if sentiment_score < 0 else max(0.7, 1.0 - sentiment_score))
    raw_score *= sentiment_factor
    source_multiplier = 1.3 if source_type == "web" else 1.0
    raw_score *= source_multiplier
    region_weight = min(weights["region_terms"].get(r, 4.0) for r in regions or ["Unknown"])
    raw_score *= (1 + (region_weight / 10))

    regulator_score = 0.0
    for state, (agencies, weight) in weights["state_regulators"].items():
    ...
    mitigation_factor = max(0.5, 1.0 - (len(mitigations) * 0.15)) if mitigations else 1.0
    raw_score *= mitigation_factor

    if tokenomics_flags:
        if tokenomics_flags.get("high_inflation", False):
            raw_score += 2.0
        if tokenomics_flags.get("low_liquidity", False):
            raw_score += 1.5

    scaled_score = min(10.0, (raw_score / 50.0) * 10.0)  # Placeholder max_raw_score; tune with data
    return {"raw_score": raw_score, "scaled_score": scaled_score}
...
def analyze_sentiment(text, sentiment_analyzer=None):
    ...
    text_lower = text.lower()
    risk_count = sum(1 for w in text_lower.split() if any(w in cat for cat in DYNAMIC_WEIGHTS["risk_categories"].values()))
...
def extract_region(text, nlp=None):
    if nlp is None:
        nlp = get_spacy_model()
    text_lower = text.lower()
    doc = nlp(text)
    ner_regions = [ent.text for ent in doc.ents if ent.label_ == "GPE"]
    term_regions = [region for region in DYNAMIC_WEIGHTS["region_terms"].keys() if re.search(r'\b' + re.escape(region.lower()) + r'\b', text_lower)]
    fuzzy_regions = []
    similarity_threshold = 85
    for candidate in ner_regions:
        if candidate.lower() not in text_lower:
            continue
        if candidate in DYNAMIC_WEIGHTS["region_terms"]:
            fuzzy_regions.append(candidate)
        else:
            for valid_region in DYNAMIC_WEIGHTS["region_terms"].keys():
...
def detect_state_risks(text):
    text_lower = text.lower()
    detected = []
    for state, (agencies, weight) in DYNAMIC_WEIGHTS["state_regulators"].items():
...
def infer_latent_risks(text, threshold=0.50, model=None):
    if model is None:
        model = get_sentence_transformer()
    embedding = model.encode(text)
    detected = []
    risk_embeddings = {cat: model.encode(list(terms)) for cat, terms in DYNAMIC_WEIGHTS["risk_categories"].items()}
...
def simulate_tokenomics(text):
    text_lower = text.lower()
    flags = {
        "high_inflation": "token inflation" in text_lower or "minting" in text_lower,
        "low_liquidity": "illiquidity" in text_lower or "liquidity drain" in text_lower,
        "vesting_schedule": "vesting" in text_lower or "lockup" in text_lower
    }
    return flags
...
def extract_entities(chunk_text, source_type="text"):
    ...
    tokenomics_flags = simulate_tokenomics(chunk_text)
    entities = {
        ...
        "tokenomics_flags": tokenomics_flags
    }
    ...
    entities["risk_score"] = calculate_risk_score(
        ...
        len(chunk_text.split()), source_type, entities["region"], chunk_text, tokenomics_flags
    )
...
def optimize_thresholds(embeddings, initial_eps=5.0, initial_threshold=0.50):
    """Dynamic tuning for DBSCAN eps and latent risk threshold."""
    scaler = StandardScaler()
    scaled_embeddings = scaler.fit_transform(embeddings)
    eps_range = np.linspace(initial_eps - 2, initial_eps + 2, 5)
    ...
    for eps in eps_range:
        ...
        if len(set(labels)) > 1:
            score = len(set(labels)) - (labels == -1).sum() / len(labels)
            if score > best_score:
                best_score = score
                best_eps = eps
    threshold = min(0.75, initial_threshold + (best_score / 10))
    return best_eps, threshold

# Precompute risk embeddings
model = get_sentence_transformer()
risk_embeddings = {cat: model.encode(list(terms)) for cat, terms in DYNAMIC_WEIGHTS["risk_categories"].items()}