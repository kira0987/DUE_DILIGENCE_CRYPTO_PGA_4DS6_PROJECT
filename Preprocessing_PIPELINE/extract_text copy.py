...
import requests
import logging
import boto3  # For AWS Textract integration

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# Initialize AWS Textract client (configure with your AWS credentials)
textract_client = boto3.client('textract', region_name='us-east-1')

def extract_text_from_pdf_with_textract(file_path):
    """Extract text from PDF using AWS Textract for high-accuracy OCR."""
    try:
...
def extract_text_to_txt(file_path, output_dir):
    """Extract text from JSON, PDF, TXT, DOCX, or HTML files with enhanced OCR and format support."""
    try:
        if file_path.endswith(".json"):
            ...
            # Fallback to AWS Textract if PyMuPDF extracts insufficient text
            ...
            # Handle multi-column layouts by converting to images if still insufficient
            if len(text.strip()) < 10:
