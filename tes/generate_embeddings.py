...
def simulate_fund_metrics(text):
    text_lower = text.lower()[:512]  # Truncate for speed
    roi = 5.0
...
def generate_embeddings(chunk_data, output_prefix, full_text=None):
    ...
    all_chunks = [entry["text"][:512] for entry in chunk_data]  # Truncate for speed
    embeddings = model.encode(all_chunks, show_progress_bar=True, batch_size=64)  # Increased batch size
    doc_embedding = model.encode(full_text[:512]) if full_text else None
