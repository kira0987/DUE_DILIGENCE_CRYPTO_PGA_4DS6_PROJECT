    ...
    if question_type == "Missing Context":
        ...
        if not context or context.startswith("❌"):
            ...
            if gap_analysis:
                ...
                if st.button("🚀 Fill Missing Gaps with External Data"):
...
# --- Auto-Answer from Question Bank Section ---
st.markdown('<p class="big-font">🗂️ Auto-Answer from Question Bank</p>', unsafe_allow_html=True)
st.markdown('<p class="medium-font">Click any question below to generate a live answer based on the uploaded document.</p>', unsafe_allow_html=True)
...
if os.path.exists(QUESTION_BANK_PATH):
    ...
    for q in questions:
        ...
        if st.button(f"❓ {q_text}", key=f"button_{q_text}"):
            with st.spinner("🔍 Classifying the question..."):
            ...
            if question_type == "Missing Context":
                ...
                if context and not context.startswith("❌"):
                    ...
                    with st.spinner("📝 Evaluating answer quality..."):
                        evaluation_raw = evaluate_answer(q_text, context, answer)
                        try:
                            evaluation_json = extract_json_from_text(evaluation_raw)
                            evaluation = json.loads(evaluation_json)
                        except Exception as e:
                    ...
                    with st.spinner("🚨 Detecting Missing Information for Scraping..."):
                        gap_raw = detect_and_structure_gaps(q_text, context, answer, evaluation.get("Missing_Points", []))
                        try:
                            gap_json = extract_json_from_text(gap_raw)
                            gap_analysis = json.loads(gap_json)
                            st.markdown("### 🛠️ Gap Analysis for Data Acquisition:")
    ...
    st.warning("⚠️ No question bank found. Please upload a valid 'question_bank.json' file.")
